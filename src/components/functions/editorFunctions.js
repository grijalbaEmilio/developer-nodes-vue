...
export function nodeSum(editor) {
    ...
    for (const key in nodes) {
        ...
        if (nodes[key].name === "sum") {
            ...
            editor.updateNodeDataFromId(nodes[key].id, { name: 0 })
            ...
            for (const i in nodes[key].inputs) {
                ...
                nodes[key].inputs[i].connections.forEach((e) => {
                    ...
                    sum += parseFloat(editor.getNodeFromId(e.node).data.name)
                    // actualizamos el valor del nodo suma 
                    editor.updateNodeDataFromId(nodes[key].id, { name: sum })
                });
            }
        }
    }
    ...
    for (const key in nodes) {
        ...
        if (nodes[key].name === "multiplication") {
            ...
            for (const i in nodes[key].inputs) {
                ...
                nodes[key].inputs[i].connections.forEach((e) => {
                    //acumulamos la suma con el valor de los nodes entrantes
                    mult *= parseFloat(editor.getNodeFromId(e.node).data.name)
                    // actualizamos el valor del nodo suma 
                    editor.updateNodeDataFromId(nodes[key].id, { name: mult })
                });
            }
        }
    }
        ...
        if (nodes[key].name === "division") {
            ...
            for (const i in nodes[key].inputs) {
                ...
                if (i === 'input_1') {
                    nodes[key].inputs[i].connections.forEach((e) => {
                        //acumulamos la suma con el valor de los nodes entrantes
                        numerator = parseFloat(editor.getNodeFromId(e.node).data.name)
                        // actualizamos el valor del nodo suma    
                    });
                }
                if (i === 'input_2') {
                    nodes[key].inputs[i].connections.forEach((e) => {
                        //acumulamos la suma con el valor de los nodes entrantes
                        divisor = parseFloat(editor.getNodeFromId(e.node).data.name)
                        // actualizamos el valor del nodo suma    
                    });
                }
            }
            editor.updateNodeDataFromId(nodes[key].id, { name: (numerator / divisor).toFixed(2) })
        }
    }
}
        ...
        if (nodes[key].name === "assign") {
            ...
            if (nodes[key].outputs['output_1'].connections[0] && nodes[key].inputs['input_1'].connections[0]) {
                ...
                if (nodeRight.name == 'vari') {
                    editor.updateNodeDataFromId(nodeRight.id, { ...nodeRight.data, value: nodeLeft.data.name })
                } else {
